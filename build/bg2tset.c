...
FILE *new_blockset_file(int block_row) {
  char *path = string_printf("out/blocks/r%x.ahi", block_row);
...
char get_tile_id(const char *tileset, int tile_index) {
  ...
  } else if (0 == strcmp(tileset, "console")) {
    return 0xbc + tile_index;
  } else {
...
void write_terrain_table(const char *label, const unsigned char *tile_ids,
                         int num_block_rows) {
  fprintf(stdout, "\n.EXPORT %s\n%s:\n", label, label);
...
int main(int argc, char **argv) {
  ...
  for (int block_row = 0; block_row < num_block_rows; ++block_row) {
    ...
    FILE *file = new_blockset_file(block_row);
    for (int block_col = 0; block_col < NUM_BLOCK_COLS; ++block_col) {
      block_t block;
  ...
  fprintf(stdout,
          ";;; This file was generated by bg2tset.\n\n"
          ".SEGMENT \"PRGA_Terrain\"\n");
  write_terrain_table("DataA_Terrain_UpperLeft_u8_arr",
                      block_nw, num_block_rows);
  write_terrain_table("DataA_Terrain_LowerLeft_u8_arr",
                      block_sw, num_block_rows);
  write_terrain_table("DataA_Terrain_UpperRight_u8_arr",
                      block_ne, num_block_rows);
  write_terrain_table("DataA_Terrain_LowerRight_u8_arr",
                      block_se, num_block_rows);
